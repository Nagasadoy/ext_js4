Ext.example = function () {
    ...
    return {
        msg: function toast(title, format) {

            if (!msgCt) {
                msgCt = Ext.DomHelper.insertFirst(document.body, { id: 'msg-div' }, true);
            }

            // Если уже есть активные тосты, то удаляем их
            // if (toast.show == true) {
            //     toast.messages.forEach(element => {
            //         document.getElementById(element.id).remove();
            //     });
            //     toast.messages = [];
            // }

            // // при первоначальной инициализации присваиваем пустой массив
            // if(toast.messages == undefined){
            //     toast.messages = [];
            // } 

            var s = Ext.String.format.apply(String, Array.prototype.slice.call(arguments, 1));
            var m = Ext.DomHelper.append(msgCt, createBox(title, s), true);
            // toast.messages.push(m);// добавляем div в список 
            m.hide();
            // toast.show = true; // тост показывается 
            m.slideIn('t').ghost("t", {
                delay: 5000,
                remove: true,
                callback: function () {
                    // toast.show = false; // тост скрыт
                }
            });
        },
